...
export const BottomNav: React.FC = () => {
  ...
  return (
    <nav
      ...
      className="md:hidden fixed bottom-0 start-0 end-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800 pb-safe transition-all shadow-lg"
    >
      <div className="grid grid-cols-5 items-center justify-items-center h-16 max-w-lg mx-auto px-2 relative">
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={`relative flex flex-col items-center justify-center w-full h-full py-1 gap-1 text-[11px] font-bold transition-all min-h-[44px] ${
                isActive
            ...
            >
              {isActive && (
                <span className="absolute top-0 inset-x-6 h-0.5 bg-teal-600 dark:bg-teal-400 rounded-full animate-in fade-in duration-200" />
              )}
              <item.icon
