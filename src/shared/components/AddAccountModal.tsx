...
import { UserPlus, Phone, FileText, Tag, UserCheck, ChevronDown } from 'lucide-react';
    ...
    <ModalShell
      ...
      title={
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-slate-100">إضافة حساب جديد</h3>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">عميل، مورد، أو شخصي</p>
          </div>
        </div>
      }
      onClose={close}
      maxWidthClass="max-w-lg"
      // keep default closeOnBackdrop behavior (true)
    >
      <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-3.5 overflow-y-auto">
        ...
        <div className="grid grid-cols-12 gap-2 sm:gap-3 items-center">
          ...
          <div className="col-span-8">
            <input
              ...
              className="w-full px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-xs sm:text-sm focus:ring-2 focus:ring-teal-500"
              autoFocus
            />
        ...
        <div className="grid grid-cols-12 gap-2 sm:gap-3 items-center">
          ...
          <div className="col-span-8">
            <div className="flex items-center rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 focus-within:ring-2 focus-within:ring-teal-500">
              <input
              ...
              <div className="relative flex items-center shrink-0 bg-slate-50/70 dark:bg-slate-800/80">
                <select
                ...
                >
                  {POPULAR_COUNTRY_CODES.map((item) => (
                    <option key={item.code} value={item.code} className="text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-900">
                      {item.label}
                    </option>
                  ))}
                </select>
        ...
        <div className="grid grid-cols-12 gap-2 sm:gap-3 items-start">
          <label htmlFor="textarea-account-note" className="col-span-4 text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5 pt-2">
          ...
          <div className="col-span-8">
            <textarea
              ...
              className="w-full px-3 py-1.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-xs sm:text-sm focus:ring-2 focus:ring-teal-500"
            />
          </div>
        </div>
        ...
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            />
            <span className="text-xs font-bold text-slate-700 dark:text-slate-300">تسجيل رصيد افتتاحي سابق</span>
          </label>
          ...
          {hasInitialBalance && (
            <div className="mt-2.5 space-y-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 animate-in fade-in duration-150">
              {/* المبلغ */}
              <div className="grid grid-cols-12 gap-2 sm:gap-3 items-center">
                <label htmlFor="input-initial-balance-amount" className="col-span-4 text-xs font-medium text-slate-600 dark:text-slate-400">
                  المبلغ ({currency})
                </label>
                <div className="col-span-8">
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-xs sm:text-sm"
                  />
                </div>
              </div>
              ...
              {/* طبيعة الرصيد */}
              <div className="grid grid-cols-12 gap-2 sm:gap-3 items-center">
                <span className="col-span-4 text-xs font-medium text-slate-600 dark:text-slate-400">طبيعة الرصيد</span>
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full py-2.5 sm:py-3 px-4 rounded-xl bg-teal-600 hover:bg-teal-700 active:bg-teal-800 disabled:opacity-50 text-white font-bold text-xs sm:text-sm flex items-center justify-center gap-2"
          >
