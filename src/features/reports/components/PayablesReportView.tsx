...
export const PayablesReportView: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        <div className="p-12 text-center text-xs font-bold text-slate-400">جاري إعداد تقرير الالتزامات...</div>
      ) : !report ? (
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 sm:p-5 rounded-3xl bg-rose-50/70 dark:bg-rose-950/30 border border-rose-200/60 dark:border-rose-800/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
            <div>
              ...
              <div className="text-2xl sm:text-3xl font-black text-rose-700 dark:text-rose-300 tabular-nums">
          ...
          <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-xs">
            {report.items.length === 0 ? (
              <div className="p-8 text-center text-xs text-slate-400">
                لا توجد ديون أو التزامات مالية مطابقة للمعايير المحددة
              </div>
            ) : (
                ...
                <table className="w-full text-xs text-right">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800 text-slate-500 font-bold">
                      ...
                      <th className="py-3 px-3 text-rose-600">المبلغ المطلوب منك</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium">
                    {report.items.map((item, idx) => (
                      <tr key={item.account.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
                        ...
                        <td className="py-2.5 px-3 font-extrabold font-mono text-rose-600 dark:text-rose-400 tabular-nums">
