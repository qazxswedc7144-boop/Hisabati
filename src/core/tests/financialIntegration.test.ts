...
export class FinancialIntegrationTestSuite {
  static async runAllTests(): Promise<{
    ...
    await run('FIN-20', 'ثبات DATABASE_SCHEMA_VERSION=12 و FINANCIAL_FORMAT_VERSION=1', async () => {
      if (DATABASE_SCHEMA_VERSION !== 12) {
        throw new Error(`DATABASE_SCHEMA_VERSION يجب أن يكون 12 حصرياً، وجد: ${DATABASE_SCHEMA_VERSION}`);
      }
      if (FINANCIAL_FORMAT_VERSION !== 1) {
        throw new Error(`FINANCIAL_FORMAT_VERSION يجب أن يكون 1 حصرياً، وجد: ${FINANCIAL_FORMAT_VERSION}`);
      }
      if (db.verno !== 12) {
        throw new Error(`إصدار Dexie الحالي يجب أن يكون 12 حصرياً، وجد: ${db.verno}`);
      }
    });
