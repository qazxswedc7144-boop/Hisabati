...
export class FinancialIntegrationTestSuite {
  static async runAllTests(): Promise<{
    ...
    await run('FIN-20', 'ثبات DATABASE_SCHEMA_VERSION=11 و FINANCIAL_FORMAT_VERSION=1', async () => {
      if (DATABASE_SCHEMA_VERSION !== 11) {
        throw new Error(`DATABASE_SCHEMA_VERSION يجب أن يكون 11 حصرياً، وجد: ${DATABASE_SCHEMA_VERSION}`);
      }
      if (FINANCIAL_FORMAT_VERSION !== 1) {
        throw new Error(`FINANCIAL_FORMAT_VERSION يجب أن يكون 1 حصرياً، وجد: ${FINANCIAL_FORMAT_VERSION}`);
      }
      if (db.verno !== 11) {
        throw new Error(`إصدار Dexie الحالي يجب أن يكون 11 حصرياً، وجد: ${db.verno}`);
      }
    });
