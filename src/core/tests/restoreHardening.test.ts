...
export class RestoreHardeningTestSuite {
  public static async runAllTests(): Promise<{
    ...
    await this.runTest(
      results,
      'RST-11',
      'عزل قاعدة البيانات: ثبات إصدارات Dexie 1-12 وثبات FINANCIAL_FORMAT_VERSION=1',
      async () => {
        ...
        if (versions && versions > 12) {
          throw new Error(`Dexie version is ${versions}, which exceeds maximum version 12!`);
        }
      }
    );
