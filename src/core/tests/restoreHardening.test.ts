...
export class RestoreHardeningTestSuite {
  public static async runAllTests(): Promise<{
    ...
    await this.runTest(
      results,
      'RST-11',
      'عزل قاعدة البيانات: ثبات إصدارات Dexie 1-11 وثبات FINANCIAL_FORMAT_VERSION=1',
      async () => {
        if (DATABASE_SCHEMA_VERSION !== 11) {
          throw new Error(`DATABASE_SCHEMA_VERSION must remain 11, got ${DATABASE_SCHEMA_VERSION}`);
        }
        ...
        if (versions && versions > 11) {
          throw new Error(`Dexie version is ${versions}, which exceeds maximum version 11!`);
        }
      }
    );
