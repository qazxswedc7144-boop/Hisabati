...
export class OCRFieldParser {
  ...
  public static parse(rawText: string, providerName: string = 'local_heuristic'): OCRResult {
    ...
    return {
      ...
      subtotalMinor: subtotal.value ? { ...subtotal, value: toMinorUnits(subtotal.value, currency.value as CurrencyCode) } : undefined,
      tax: tax.value ? tax : undefined,
      taxMinor: tax.value ? { ...tax, value: toMinorUnits(tax.value, currency.value as CurrencyCode) } : undefined,
      totalAmount,
      totalAmountMinor: {
        ...totalAmount,
        value: totalAmount.value ? toMinorUnits(totalAmount.value, currency.value as CurrencyCode) : 0,
      },
      lineItems,
      rawText: cleanText,
      overallConfidence,
