...
export class FinancialIntegrityService {
  ...
  async verifyFinancialIntegrity(): Promise<IntegrityReport> {
      ...
      if (trx.amountMinor !== undefined) {
        ...
        if (!dualCheck.isValid) {
          inconsistencies.push({
            type: 'invalid_amount',
            entityId: trx.id,
            details: `عدم تطابق القيمة الصغرى للعملية (amountMinor): ${dualCheck.error}`,
            expected: String(dualCheck.expectedMinor),
            actual: String(trx.amountMinor),
          });
        }
      }
    ...
    for (const account of allAccounts) {
      ...
      if (account.currentBalanceMinor !== undefined && account.currentBalanceMinor !== computed.currentBalanceMinor) {
        inconsistencies.push({
          type: 'balance_mismatch',
          entityId: account.id,
          details: `عدم تطابق الرصيد المخزن بالوحدات الصغرى (currentBalanceMinor) للحساب "${account.name}"`,
          expected: computed.currentBalanceMinor,
          actual: account.currentBalanceMinor,
        });
      }
    }
